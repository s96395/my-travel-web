...
function renderHeader(data) {
    ...
    document.getElementById('hero-banner').style.backgroundImage = `linear-gradient(rgba(26,58,95,0.7), rgba(26,58,95,0.7)), url('${data.coverImageUrl || 'https://images.unsplash.com/photo-1488646953014-85cb44e25828'}')`;
}
...
function setupEvents() {
    ...
    document.getElementById('addDayBtn').onclick = () => open("新增行程", `
        ...
        <div class="form-group"><label>活動內容</label><input type="text" name="activity" required></div>
...
async function loadAllData() {
    ...
    sI.forEach(d => {
        const item = d.data();
        if(lastDay !== item.day) { lastDay = item.day; htmlI += `<h3 style="margin:20px 0 10px; border-left:5px solid var(--primary); padding-left:10px;">第 ${lastDay} 天</h3>`; }
        htmlI += `<div class="itinerary-item">
                    <div><span style="color:var(--accent);">${item.time || '--:--'}</span> <strong>${item.activity}</strong></div>
                    <button class="delete-btn-item" onclick="deleteSubItem('itinerary', '${d.id}')">×</button>
                  </div>`;
    });
    ...
    sE.forEach(d => {
        ...
        htmlE += `<div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <span>💸 ${ex.name}</span>
    ...
    let htmlPh = "";
    sPh.forEach(d => {
        htmlPh += `<div style="position:relative; aspect-ratio:1; overflow:hidden; border-radius:10px;">
                    <img src="${d.data().url}" style="width:100%;height:100%;object-fit:cover;">
                    <button onclick="deleteSubItem('images', '${d.id}')" style="position:absolute; top:5px; right:5px; background:rgba(255,255,255,0.7); border:none; border-radius:50%; width:24px; height:24px;">×</button>
                   </div>`;
    });
